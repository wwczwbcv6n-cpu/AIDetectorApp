...
/**
 * JVM (desktop) primitives. Mirrors `Primitives.android.kt` byte for byte —
 * keep the two in sync; the desktop unit tests are what actually exercise
 * this code against RFC 9180 A.1 and `secure/vectors.json`.
 *
 * BouncyCastle is used through its LIGHTWEIGHT API only (no
 * `Security.addProvider`), because Android ships a stripped provider that is
 * also called "BC" and registering the real one there is a known trap.
 */
